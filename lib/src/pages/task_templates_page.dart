...
class TaskTemplatesPage extends StatelessWidget {
  ...
  static const _roles = [
    'Civil','Owner','Surveyor','Architect','MEP','Structural','Geotechnical','Landscape','Other'
  ];
      ...
      body: StreamBuilder<List<TaskTemplate>>(
        ...
        builder: (context, snap) {
          ...
          for (final t in items) {
            final phase = (t.taskCode.length >= 2) ? t.taskCode.substring(0, 2) : '00';
          ...
          return ListView.builder(
            ...
            itemCount: orderedPhaseKeys.length + 1, // +1 for bottom "New" button
              ...
              return Card(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(_phaseLabel(phase), style: Theme.of(context).textTheme.titleMedium),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            FilledButton.icon(onPressed: onAdd, icon: const Icon(Icons.add), label: const Text('New Template Task')),
            const SizedBox(height: 8),
            TextButton.icon(onPressed: onSeed, icon: const Icon(Icons.playlist_add), label: const Text('Seed starter list')),
          ],
        ),
      ),
    );
            ...
            child: SizedBox(
              ...
              child: SingleChildScrollView(
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  appTextField('Task Code (4 digits)', codeCtl, required: true, hint: 'e.g., 0101'),
                  const SizedBox(height: 10),
                  appTextField('Task Name', nameCtl, required: true, hint: 'e.g., Zoning Research'),
                  const SizedBox(height: 10),
                  ...
                  const SizedBox(height: 10),
                  appTextField('Default Project Number', projNumCtl, hint: 'optional (e.g., 24017)'),
                ]),
              ),
            ),
          ),
            ...
            FilledButton(
              onPressed: () async {
                ...
                final t = TaskTemplate(
                  ...
                  taskCode: code,
                  taskName: name,
                  taskNote: noteCtl.text.trim().isEmpty ? null : noteCtl.text.trim(),
                  taskResponsibility: responsibility,
                  isDeliverable: isDeliverable,
                  projectNumber: projNumCtl.text.trim().isEmpty ? null : projNumCtl.text.trim(),
                  ownerUid: ownerUid,
                );
                await repo.add(t);
        ...
        return AlertDialog(
          ...
          content: Form(
            ...
            child: SizedBox(
              ...
              child: SingleChildScrollView(
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  appTextField('Task Code (4 digits)', codeCtl, required: true),
                  const SizedBox(height: 10),
          ...
          actions: [
            TextButton(
              onPressed: () async {
                final ok = await confirmDialog(context, 'Delete this template task?');
                if (!ok) return;
...
Future<void> _seedStarterList(BuildContext context, String ownerUid) async {
  ...
  final starters = <TaskTemplate>[
    // 01 — Land Use
    TaskTemplate(id: '_', taskCode: '0101', taskName: 'Zoning Research', taskNote: 'Confirm zoning, overlays, variances', taskResponsibility: 'Civil', isDeliverable: false, ownerUid: ownerUid),
    TaskTemplate(id: '_', taskCode: '0102', taskName: 'Utility Locator (811)', taskNote: 'Request locates; map conflicts', taskResponsibility: 'Civil', isDeliverable: false, ownerUid: ownerUid),
    TaskTemplate(id: '_', taskCode: '0103', taskName: 'Pre-App Meeting', taskNote: 'Schedule/attend with AHJ', taskResponsibility: 'Owner', isDeliverable: false, ownerUid: ownerUid),

    // 02 — Preliminary Design
    TaskTemplate(id: '_', taskCode: '0201', taskName: 'Topographic Survey', taskNote: 'Obtain topo + boundary', taskResponsibility: 'Surveyor', isDeliverable: true, ownerUid: ownerUid),
    TaskTemplate(id: '_', taskCode: '0202', taskName: 'Concept Site Plan', taskNote: 'Layout and grading concept', taskResponsibility: 'Civil', isDeliverable: true, ownerUid: ownerUid),
    TaskTemplate(id: '_', taskCode: '0203', taskName: 'Geotechnical Report', taskNote: 'Soils borings + recommendations', taskResponsibility: 'Geotechnical', isDeliverable: true, ownerUid: ownerUid),

    // 03 — Construction Design
    TaskTemplate(id: '_', taskCode: '0301', taskName: 'Civil CDs', taskNote: 'Grading, drainage, utilities', taskResponsibility: 'Civil', isDeliverable: true, ownerUid: ownerUid),
    TaskTemplate(id: '_', taskCode: '0302', taskName: 'Architectural CDs', taskNote: 'Coordinate site/building', taskResponsibility: 'Architect', isDeliverable: true, ownerUid: ownerUid),
    TaskTemplate(id: '_', taskCode: '0303', taskName: 'MEP CDs', taskNote: 'Utility connections + loads', taskResponsibility: 'MEP', isDeliverable: true, ownerUid: ownerUid),

    // 04 — Construction Management
    TaskTemplate(id: '_', taskCode: '0401', taskName: 'Preconstruction Meeting', taskNote: 'Contractor kickoff', taskResponsibility: 'Owner', isDeliverable: false, ownerUid: ownerUid),
    TaskTemplate(id: '_', taskCode: '0402', taskName: 'RFIs & Submittals', taskNote: 'Track and respond', taskResponsibility: 'Civil', isDeliverable: false, ownerUid: ownerUid),
    TaskTemplate(id: '_', taskCode: '0403', taskName: 'As-Builts', taskNote: 'Record drawings', taskResponsibility: 'Surveyor', isDeliverable: true, ownerUid: ownerUid),
  ];
